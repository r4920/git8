...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('UIK91xum2goloh_') ) {
      let user = new User({
        'password':'UIK91xum2goloh_',
        'username':'Stacy63',
    ...
    if (!admin || !admin.isPasswordMatch('529oZoWN1TrCViV') ) {
      let admin = new User({
        'password':'529oZoWN1TrCViV',
        'username':'Russel69',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('UIK91xum2goloh_') && userRole){
    ...
    if (admin && admin.isPasswordMatch('529oZoWN1TrCViV') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
